...
```{r}
dall = read_rds("ehama_dataset.rds")
dall = dall |> 
...
七目における環境要因の特徴は焼崎と江袋と異なり、焼崎と江袋の環境要因の特徴は似ていると推測しました。
解析結果はあくまでも暫定的であり、環境要因を毎月にモニタリングするは必要です。
さらに、植生調査を行うことも重要です。
...
水温、塩分、全リン、全窒素、クロロフィル濃度の観測頻度は場所と年で異なり、１回から６回程度は観測されています。
水深、溶存酸素濃度、NO~2~、NO~3~、NH~4~ 、PO~4~ も観測されているが、本解析に使用していません。
NO~2~、NO~3~、NH~4~ 、PO~4~は全窒素および全リンに含まれているから外しました。
水深と溶存酸素濃度は場所と測定手法によってことなりため、外しました。

規則的な変動は水温に見られたが、のこりの環境要因には目立ったパターンはありませんでした (図 \@ref(fig:envplot))。
...
write_rds(bout, "multivariate_model.rds")
bout = read_rds("multivariate_model.rds")
```
...
多変量分散分析の結果 (図 \@ref(fig:manova)) により、七目の水温と塩分は他の他地点より低く、クロロフィル、全リン、全窒素は他地点より高いことが示されました。
...
多次元尺度法を用いてい、環境要因の特徴を年ごごとに示しました（図 \@ref(fig:mds))。
楕円は各地点における座標値の95%信頼区間を示しています。
焼崎と江袋の座標値に目立った違いがなく、2013年から2021年において環境要因の特徴に違いはないと考えられます。
ところが、七目は焼崎と江袋から比較的に離れていて、その違いは毎年現れています。

七目の植生は、焼崎と江袋より状態であり、特に今年 (2021) はアマモ、ヒジキ、アカモクなどの大型藻類が七目に繁茂していました。
丁寧に生態系の調査を行い、本結果の検証は必要だと考えられますが、環境要因の特徴に違いがあると考えられます。
